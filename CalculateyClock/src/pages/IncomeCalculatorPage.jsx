...
function StandaloneCaliforniaCalculator() {
  ...
  const socialSecurityTax = Math.min(taxableIncome * 0.062, 9932.40);
    ...
    <GradientBackground>
      <div className="flex flex-col flex-grow">
        ...
        <div className="bg-white shadow-lg rounded-lg p-6 max-w-4xl mx-auto mt-6 flex-grow">
          <h1 className="text-2xl font-bold mb-6 text-center text-gray-800">California Income Calculator</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setRetirement401k(Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)))}
                className="w-full p-2 border border-gray-300 rounded"
              />
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            <div className="grid grid-cols-2 gap-2 mb-4">
              <div>
                ...
                <ResultRow label="401(k) Contribution:" value={contribution401k} />
            ...
            <div className="border-t border-gray-300 pt-2 mt-2">
              <ResultRow label="Total Taxes:" value={totalTaxes} isBold={true} />
            </div>
...
function ResultRow({ label, value, isBold = false, isGreen = false }) {
  return (
    <div className={`flex justify-between ${isBold ? 'font-bold' : ''}`}>
      <span>{label}</span>
      <span className={isGreen ? 'text-green-600' : ''}>${value.toFixed(2)}</span>
    </div>
  );
}
