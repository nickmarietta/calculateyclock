...
function calculateEarnings(hours, hourlyWage, retirement401k = 5) {
  ...
  const totalTaxes = californiaTax + federalTax + socialSecurityTax + medicareTax;
...
{/*----------------------------------------Math To Calculate Hours And Time From Time Sheet ---------------*/}
  const totalHours = safeShifts.reduce((sum, shift) => sum + parseFloat(shift.hours), 0);
  const earningsSummary = calculateEarnings(totalHours, hourlyWage, retirement401k);
  ...
  const events = safeShifts.map((shift, index) => {
    ...
    const earnings = calculateEarnings(parseFloat(shift.hours), hourlyWage, retirement401k);
...
{/*----------------------------------------Edit/Delete Shifts ---------------------------------------------------------*/}
  ...
  const calculateHours = () => {
    try {
      const startTime = moment(`${editFormData.date} ${editFormData.clockIn}`, "YYYY-MM-DD HH:mm");
      const endTime = moment(`${editFormData.date} ${editFormData.clockOut}`, "YYYY-MM-DD HH:mm");
        ...
        <main className="flex-grow pt-20 px-4 pb-10">
          <div className="bg-white/90 backdrop-blur-sm shadow-lg rounded-lg p-6 max-w-5xl mx-auto mt-6">
            ...
            <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="block text-gray-700 mb-2">Hourly Wage ($)</label>
                <input
                  type="number"
                  value={hourlyWage}
              ...
              <div>
                <label className="block text-gray-700 mb-2">401(k) Contribution (%)</label>
                <input
                  type="number"
                  value={retirement401k}
                  onChange={(e) => 
                    setRetirement401k(Math.min(100, Math.max(0, parseFloat(e.target.value) || 0)))
                  }
                  className="w-full p-2 border border-gray-300 rounded"
                />
              </div>
            ...
            {safeShifts.length > 0 && (
              <div className="mb-6 bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <ResultRow label="Total Hours:" value={totalHours} isBold={true} />
                    <ResultRow label="Gross Income:" value={earningsSummary.grossIncome} />
                    <ResultRow label="401(k) Contribution:" value={earningsSummary.contribution401k} />
                  </div>
                  <div>
                    <ResultRow label="Total Taxes:" value={earningsSummary.totalTaxes} />
                    <ResultRow label="Net Income:" value={earningsSummary.netIncome} isBold={true} isGreen={true} />
                  </div>
                </div>
              </div>
            )}
                    ...
                    >
                      <div className="flex justify-between items-center mb-2">
                        <div>
                          ...
                          <p><strong>Hours:</strong> {shift.hours} ({shift.clockIn} - {shift.clockOut})</p>
                        </div>
                        <div className="text-right">
                          <p><strong>Gross:</strong> ${earnings.grossIncome.toFixed(2)}</p>
                          <p className="text-green-600 font-bold">
                            <strong>Net:</strong> ${earnings.netIncome.toFixed(2)}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex gap-2 mb-2">
                        ...
                        >
                          {showEarningsDetails[index] ? 'Hide Details' : 'Show Details'}
                        </button>
                        <button
                      ...
                      {showEarningsDetails[index] && (
                        <div className="mt-2 pt-2 border-t text-sm">
                          <div className="grid grid-cols-2 gap-2">
                            <div>
                              <ResultRow label="401(k) Contribution:" value={earnings.contribution401k} />
                              <ResultRow label="California Tax:" value={earnings.californiaTax} />
                              <ResultRow label="Federal Tax:" value={earnings.federalTax} />
                            </div>
                            <div>
                              <ResultRow label="Social Security:" value={earnings.socialSecurityTax} />
                              <ResultRow label="Medicare:" value={earnings.medicareTax} />
                              <ResultRow label="Total Taxes:" value={earnings.totalTaxes} isBold={true} />
                            </div>
                          </div>
                        </div>
                      )}
              ...
              <div className="relative z-10 bg-white rounded shadow p-4" style={{ height: "650px" }}>
                <Calendar
                  ...
                  tooltipAccessor={(event) => {
                    ...
                    return `Hours: ${event.shift.hours}\nGross: ${earnings.grossIncome.toFixed(2)}\nNet: ${earnings.netIncome.toFixed(2)}`;
                  }}
                  onSelectEvent={(event) => {
{/*----------------------------------------Open Edit Model---------------------------------------------------------*/}
            ...
            <div className="bg-white rounded-lg p-6 max-w-md w-full">
              ...
              <form onSubmit={handleEditFormSubmit}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  ...
                  <div>
                    <label className="block text-gray-700 mb-2">Clock Out</label>
                    <input
                      type="time"
                      name="clockOut"
